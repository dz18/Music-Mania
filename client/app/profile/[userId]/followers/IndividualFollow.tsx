...
export function IndividualFollow ({
  follower, isFollowingMap, follow, unfollow, index
} : {
  follower:  {
    createdAt: Date;
    follower: {
  ...
  const initialAvatar = follower.follower.id
    ? `${process.env.NEXT_PUBLIC_AWS_S3_BASE_URL}/avatars/${follower.follower.id}`
    : defaultAvatar
  ...
  return (
    ...
    >
      ...
      <div className="text-sm grow">
        <p className="font-mono font-bold hover:underline cursor-pointer" onClick={() => router.push(`/profile/${follower.followerId}`)}>
          {follower.follower.username}
        </p>
        <p className="text-gray-400">Since {timeAgo(follower.createdAt)}</p>
      </div>

      {session &&
        session?.user.id !== follower.followerId &&
          <div className="flex items-center">
            <button 
              className={`
                ${isFollowingMap[follower.followerId] ? 
              ...
              `}
              onClick={() => isFollowingMap[follower.followerId] ? unfollow(follower.followerId) : follow(follower.followerId)}
            >
              {isFollowingMap[follower.followerId] ? 
                <>
